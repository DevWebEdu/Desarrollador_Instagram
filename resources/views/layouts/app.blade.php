            ...
            @auth
                <nav class="flex gap-2 items-center">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button cass="font-bold uppercase text-gray-600 text-sm "> Cerrar Sesión </button>
                    </form>

                </nav>
            @endauth
            {{-- VERIFICANDO CON @GUEST DIRECTIVA  SI EL USUARIO ESTA AUTENTICADO PARA MOSTRAR SOLO BOTON DE CERRAR SESION --}}
            @guest
                <nav class="flex gap-2 items-center">
